...
public class hello
{
    static public class windowmesenge
    {
        ...
        {
            File.WriteAllText("exe", Path.GetDirectoryName(Application.dataPath) + @"\windows\" + app + ".exe");
            Process p = new Process();
            p.StartInfo.FileName = Path.GetDirectoryName(Application.dataPath) + @"\windows\" + app + ".exe";
            if (File.Exists(Path.GetDirectoryName(Application.dataPath) + @"\windows\" + app + ".exe"))
            {
                p.Start();
            }
        }
